...
function extractNameFromTemplate(value) {
    let arr = value.replace(/hello/i, '').replace(/,/, '').replace(/!/, '').trim();
    return arr
}
...
function removeLeadingAndTrailingWhitespaces(value) {
    return value.replace(/[^a-zA-Z!, ]/g, "").replace(/ +/g, ' ').trim()
}
...
function getCardId(value) {
    ...
    return suit.indexOf(value.slice(-1)) * 13 + number.indexOf(value.slice(0, 1))
}
